...
function checkOrderResponse(result)
{
  ...
  try
  {
	var wait_orders = result.wait_orders ? result.wait_orders :0;
	var new_orders = result.new_orders ? result.new_orders :0;
	var new_paid = result.new_paid ? result.new_paid :0;
	var user_account = result.user_account ? result.user_account :0;//未处理充值提现申请
	var seller_account = result.seller_account ? result.seller_account :0;//未处理商家实名认证
	var shop_account = result.shop_account ? result.shop_account :0;//未审核商家
	var shopinfo_account = result.shopinfo_account ? result.shopinfo_account :0;//未审核店铺信息
	var no_paid = result.no_paid ? result.no_paid :0;//待发货订单
	var no_change = result.no_change ? result.no_change :0;//待处理退换货订单
	var wait_cash = result.wait_cash ? result.wait_cash :0;//待审核商家提现
	var advance_date = result.advance_date ? result.advance_date :0;//即将过期广告
	var goods_report = result.goods_report ? result.goods_report :0;//未处理商品投诉
	var complaint = result.complaint ? result.complaint :0;//未完成交易投诉
	...
	var total = parseInt(new_orders)+parseInt(new_paid)+parseInt(user_account)+parseInt(shop_account)+parseInt(shopinfo_account)+parseInt(no_paid)+parseInt(no_change)+parseInt(wait_cash)+parseInt(advance_date)+parseInt(goods_report)+parseInt(complaint)+parseInt(seller_account);
	 ...
	 $("#total").html(total);
	 $("#new_orders").html(new_orders);
	 $("#new_paid").html(new_paid);
	 $("#shop_account").html(shop_account);
	 $("#shopinfo_account").html(shopinfo_account);
	 $("#user_account").html(user_account);
	 $("#no_paid").html(no_paid);
	 $("#no_change").html(no_change);
	 $("#wait_cash").html(wait_cash);
	 $("#advance_date").html(advance_date);
	 $("#goods_report").html(goods_report);
